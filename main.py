...
pid_pitch = PID(50, 3, 0.3, setpoint=pitch_deg_setpoint, auto_mode= True, sample_time=0.01)
...
def ctrls_callback(ctrls_data, event_pipe):
    ...
    if event_pipe.child_poll():
        child_aileron = event_pipe.child_recv()
        child_elevator = event_pipe.child_recv()
        child_rudder = event_pipe.child_recv()
        ...
        #  this on the backend
        child_aileron_state = child_aileron[0]
        child_elevator_state = child_elevator[0]
        child_rudder_state = child_rudder[0]
...
if __name__ == '__main__':  # NOTE: This is REQUIRED on Windows!
    ...
    while True:
        if fdm_event_pipe.parent_poll():  # Only update controller when we get FDM updates
            ...
            if (yaw_error > 0.0):
                parent_rudder_req = yaw_error * (pid_yaw(parent_yaw_deg)*0.001)
            if (yaw_error < 0.0):
                parent_rudder_req = yaw_error * (pid_yaw(parent_yaw_deg)*-0.001)
            
            print(parent_elevator_req)
